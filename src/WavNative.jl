...
function audioread(data::Vector{UInt8}, meta::WavMetadata{nbits, nchans}, ::Type{T}) where {nbits, nchans, T}
    if nbits != 24 && sizeof(T) * 8 == nbits
        ...
        # Use Sample{nchans, T} for everything > 1 channel
        final_view = nchans == 1 ? raw_view : reinterpret(Sample{nchans, T}, raw_view)
        return final_view, meta.sample_rate
    else
        # Determine the target type for allocation
        FinalType = nchans == 1 ? T : Sample{nchans, T}
...
function _process_bits!(dest::AbstractVector{T}, raw::Vector{UInt8}, meta::WavMetadata{nbits, nchans}) where {T, nbits, nchans}
    # 1. Change: Check for Sample instead of Channels
    ...
    @inbounds @simd for i in 1:length(dest)
        ...
        samples_tuple = ntuple(nchans) do ch
            ...
            elseif nbits == 32
                val = reinterpret(Int32, @view(raw[off:off+3]))[1]
        ...
        # 2. Change: Check for Sample instead of Channels
        # Note: samples_tuple[1] is used for Mono (nchans=1)
        dest[i] = T <: Sample ? T(samples_tuple...) : samples_tuple[1]
    end
    return dest
end
