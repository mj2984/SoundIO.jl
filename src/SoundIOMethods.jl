...
function get_sound_device_parameters(c_dev::SoundDevice_C)
    formats = get_sound_device_parameter(Cint,c_dev.formats,c_dev.format_count)
    layouts = get_sound_device_parameter(SoundDeviceChannelLayout,c_dev.layouts,c_dev.layout_count)
...
@inline userdata_offset(::Type{InputSoundStream})  = SOUNDDEVICE_INPUTSTREAM_USERDATA_OFFSET
@inline userdata_offset(::Type{OutputSoundStream}) = SOUNDDEVICE_OUTPUTSTREAM_USERDATA_OFFSET
...
@inline negotiate_callback_buffer_space_base!(areas_ref::Ref{Ptr{SoundDeviceChannelArea_C}},frames_ref::Ref{Cint},stream_ptr::Ptr{OutputSoundStream}) = ccall((:soundio_outstream_begin_write,libsoundio), Cint, (Ptr{Cvoid}, Ptr{Ptr{SoundDeviceChannelArea_C}}, Ptr{Cint}), stream_ptr, areas_ref, frames_ref)
@inline negotiate_callback_buffer_space_base!(areas_ref::Ref{Ptr{SoundDeviceChannelArea_C}},frames_ref::Ref{Cint},stream_ptr::Ptr{InputSoundStream}) = ccall((:soundio_instream_begin_read,libsoundio), Cint, (Ptr{Cvoid}, Ptr{Ptr{SoundDeviceChannelArea_C}}, Ptr{Cint}), stream_ptr, areas_ref, frames_ref)
@inline function negotiate_callback_buffer_space(stream_ptr::Ptr{StreamBaseType},requested_frames::Cint) where {StreamBaseType}
    areas_ref = Ref{Ptr{SoundDeviceChannelArea_C}}()
    ...
    if(StreamBaseType === InputSoundStream)
        ...
        return unsafe_load(areas_ref[]).ptr, Int(frames_ref[]::Cint) # Note: unsafe_load(areas_ref[]) returns a SoundDeviceChannelArea_C
    end
end
...
function make_audio_callback(::Type{StreamBaseType},::Type{BufType}, callback_function::F) where {StreamBaseType,BufType<:SoundDeviceSynchronizer, F<:Function}
...
function open_unsafe!(ctx::SoundDeviceContext)
    ...
    ctx.ptr[] == C_NULL && error("Failed to re-allocate Sound Device context.")
    return
end
function open!(ctx::SoundDeviceContext)
...
# Severing
disconnect_unsafe!(ctx::SoundDeviceContext) = ccall((:soundio_disconnect, libsoundio), Cvoid, (Ptr{Cvoid},), ctx.ptr[])
function disconnect!(ctx::SoundDeviceContext)
    if isopen(ctx) && is_connected(ctx)
        disconnect_unsafe!(ctx)
    end
end
# Cleanup
function Base.close(ctx::SoundDeviceContext)
    if isopen(ctx)
        disconnect!(ctx)
        empty!(ctx.devices)
...
# Event Management
flush_events_unsafe!(ctx::SoundDeviceContext) = ccall((:soundio_flush_events, libsoundio), Cvoid, (Ptr{Cvoid},), ctx.ptr[])
flush_events!(ctx::SoundDeviceContext) = isopen(ctx) && flush_events_unsafe!(ctx)
# Blocking Wait
# Note: soundio_wait_events blocks the thread until an event occurs.
@inline SoundDevice_wait_unsafe(ctx_ptr) = ccall((:soundio_wait_events,libsoundio), Cvoid, (Ptr{Cvoid},), ctx_ptr)
@inline wait_unsafe(device::SoundDevice) = SoundDevice_wait_unsafe(device.ptrs[].ctx)
@inline wait_unsafe(ctx::SoundDeviceContext) = SoundDevice_wait_unsafe(ctx.ptr[])
...
function get_device_count_and_offset(ctx::SoundDeviceContext, isinput::Val{true})
...
function get_device_count_and_offset(ctx::SoundDeviceContext, isinput::Val{false})
...
get_device_ptr(ctx::SoundDeviceContext, offset::Int, isinput::Val{true}) = ccall((:soundio_get_input_device, libsoundio), Ptr{Cvoid}, (Ptr{Cvoid}, Cint), ctx.ptr[], offset)
get_device_ptr(ctx::SoundDeviceContext, offset::Int, isinput::Val{false}) = ccall((:soundio_get_output_device, libsoundio), Ptr{Cvoid}, (Ptr{Cvoid}, Cint), ctx.ptr[], offset)
function insert_device!(devices::SoundDevices, dev::SoundDevice{S, Access}) where {S, Access}
...
function enumerate_devices_unsafe_internal!(ctx::SoundDeviceContext,::Val{isinput}) where isinput
    ...
    for offset in 0:(device_count - 1)
        ...
        # if dev_ptr != C_NULL
        # try
        dev = SoundDevice(ctx.ptr[], device_ptr, offset == default_device_offset)
...
list_devices(ctx::SoundDeviceContext,::RawSoundAccess) = ctx.devices.raw
list_devices(ctx::SoundDeviceContext,::SharedSoundAccess) = ctx.devices.shared
list_devices(ctx::SoundDeviceContext) = list_devices(ctx,rawsoundaccess)
initialize_sound_stream_base(device::SoundDevice{InputSoundStream,Mode}) where {Mode} = ccall((:soundio_instream_create, libsoundio), Ptr{InputSoundStream}, (Ptr{Cvoid},), device.ptrs[].device)
initialize_sound_stream_base(device::SoundDevice{OutputSoundStream,Mode}) where {Mode} = ccall((:soundio_outstream_create, libsoundio), Ptr{OutputSoundStream}, (Ptr{Cvoid},), device.ptrs[].device)
function initialize_sound_stream(device::SoundDevice{StreamBaseType,Mode}) where {StreamBaseType,Mode}
...
function open_sound_stream_error_check(result::Cint)
    ...
    err_sym = get(SoundDeviceErrorMap, result, :UnknownError) # Get our clean Julia symbol
    ...
    error("SoundDevice [:$err_sym]: $c_msg (Code: $result)")
end
...
function open_sound_stream(device_configuration::SoundDeviceConfiguration{StreamBaseType,Access}, buffer::T, callback::Base.CFunction, preserve::Any) where {StreamBaseType,Access,T <: SoundDeviceSynchronizer}
    ...
    s.layout, s.format, s.sample_rate, s.userdata, s.software_latency = device_configuration.layout, device_configuration.format, device_configuration.sample_rate, pointer_from_objref(buffer_ref), device_configuration.latency
    ...
    stream = SoundDeviceStream{StreamBaseType,T}(out_ptr, s.format, s.sample_rate, buffer_ref, callback, preserve) #latency_seconds, actual_s.software_latency
    push!(device.streams, stream) 
    return stream
end
function open_sound_stream(device_configuration::SoundDeviceConfiguration{StreamBaseType,Access}, buffer::T, callback_function::F, preserve::Any) where {StreamBaseType, Access, T <: SoundDeviceSynchronizer, F <: Function}
...
function get_destination_format(::Type{T}) where T
    ...
    T === Float32 && return SoundDeviceFormats[:Float32Little]
    T === Float64 && return SoundDeviceFormats[:Float64Little]
...
@inline start_base(stream::SoundDeviceStream{InputSoundStream, T}) where {T} = ccall((:soundio_instream_start, libsoundio), Cint, (Ptr{Cvoid},), stream.ptr)
@inline start_base(stream::SoundDeviceStream{OutputSoundStream, T}) where {T} = ccall((:soundio_outstream_start, libsoundio), Cint, (Ptr{Cvoid},), stream.ptr)
function start!(stream::SoundDeviceStream{S, T}) where {S,T}
...
=#
@inline destroy_sound_stream_unsafe(stream::SoundDeviceStream{InputSoundStream, T}) where {T} = ccall((:soundio_instream_destroy, libsoundio), Cvoid, (Ptr{Cvoid},), stream.ptr)
@inline destroy_sound_stream_unsafe(stream::SoundDeviceStream{OutputSoundStream, T}) where {T} = ccall((:soundio_outstream_destroy, libsoundio), Cvoid, (Ptr{Cvoid},), stream.ptr)
@inline function destroy_sound_stream!(device::SoundDevice,stream_enumeration::Int)
